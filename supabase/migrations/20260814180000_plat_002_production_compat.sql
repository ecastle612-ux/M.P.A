-- PLAT-002 / ADR-027 production compatibility successor (docs/97)
-- Idempotent. Contains approved ADR-026 helpers/policies plus:
--   1. Conditional maintenance_notifications policy (skip if relation absent)
--   2. DROP leftover maintenance_work_orders_*_authorized policies
-- Do not create maintenance_notifications. Do not replay J6 / FO lineage.
-- Do not re-apply 20260814160000 as-is on Production.
-- Additive. No Stripe / SKU / entitlement-key / role catalog / row deletes.
...
-- ---------------------------------------------------------------------------
-- Conditional notifications (ADR-027). Do not create the table.
-- ---------------------------------------------------------------------------

do $plat002_notifications$
begin
  if to_regclass('public.maintenance_notifications') is not null then
    execute 'drop policy if exists maintenance_notifications_insert on public.maintenance_notifications';
    execute $plat002_notify_policy$
      create policy maintenance_notifications_insert on public.maintenance_notifications
      for insert with check (
        public.is_maintenance_manager(organization_id)
        or user_id = auth.uid()
      )
    $plat002_notify_policy$;
  end if;
end
$plat002_notifications$;
...
-- ---------------------------------------------------------------------------
-- Drop leftover FO-enablement policies (no work_surface; permissive OR bypass)
-- ---------------------------------------------------------------------------
