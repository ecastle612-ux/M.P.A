...
-- Dry-run: finance_m2_run(true) validates and returns source totals only.
...
begin
  ...
  return query
  select rc.organization_id, 'error', 'unsupported_currency', rc.id::text
  from public.rent_charges rc
  where (p_organization_id is null or rc.organization_id = p_organization_id)
  ...
  return query
  select p.organization_id, 'error', 'unsupported_currency', p.id::text
  from public.payments p
  where (p_organization_id is null or p.organization_id = p_organization_id)
...
begin
  ...
  loop
    raise exception '%:%', issue.code, issue.detail;
  end loop;
  ...
  if p_dry_run then
    return jsonb_build_object(
      'organization_id', p_organization_id,
      'dry_run', true,
      'source_charges', source_charge_count,
      'source_payments', source_payment_count,
      'source_charge_total', source_charge_total,
      'source_paid_total', source_paid_total
    );
  end if;
  ...
  loop
    ...
    if exists (select 1 from public.lease_agreements la where la.id = lease_row.id) then
      ...
      unit_ok := null;
      if lease_row.unit_id is not null and exists (
        select 1 from public.property_units u where u.id = lease_row.unit_id
      ) then
        unit_ok := lease_row.unit_id;
      end if;
  ...
  loop
    ...
    if pm_id is null and exists (select 1 from public.pm_residents pr where pr.id = tenant_row.id) then
      ...
      unit_ok := coalesce(
        case when exists (select 1 from public.property_units u where u.id = tenant_row.finance_unit_id)
          then tenant_row.finance_unit_id end,
        case when exists (select 1 from public.property_units u where u.id = tenant_row.unit_id)
          then tenant_row.unit_id end
      );
      if unit_ok is null then
        raise exception 'missing_unit_for_resident:%', tenant_row.id;
      end if;
  ...
  loop
    ...
    unit_ok := case
      when exists (select 1 from public.property_units u where u.id = charge_row.unit_id)
      then charge_row.unit_id
    end;
  ...
  return jsonb_build_object(
    ...
    'charge_total', target_charge_total,
    'paid_total', target_paid_total,
