...
describe("COM-002 Slice B demo foundation", () => {
  ...
  it("routes conversion into the Get Started acquisition funnel", () => {
    ...
    expect(demoConversionHref("mpa_property_manager", "start_subscription", "demo_x")).toContain(
      "/get-started?intent=mpa_property_manager"
    );
    expect(demoConversionHref("mpa_property_manager", "start_subscription", "demo_x")).toContain(
      "demo_session_id=demo_x"
    );
    expect(demoConversionHref("mpa_facility_operations", "start_subscription")).toContain(
      "/get-started?intent=mpa_facility_operations"
    );
    expect(demoConversionHref("mpa_property_manager", "start_subscription")).not.toContain("/checkout?");
