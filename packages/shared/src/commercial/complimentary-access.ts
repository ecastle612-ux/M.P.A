...
export const MASTER_ADMIN_GRANT_STATUSES = ["INVITED", "ACTIVE", "EXPIRED", "REVOKED"] as const;
...
export const MASTER_ADMIN_GRANT_AUDIT_EVENTS = {
  ...
  ACTIVATED: "MASTER_ADMIN_GRANT_ACTIVATED",
...
/** Paid-tier entitlements only when lifecycle is ACTIVE and inside the time window. */
export function isEntitlementActiveComplimentaryGrant(
...
/** Open grant (INVITED or ACTIVE) that has passed expiration_date. */
export function isComplimentaryGrantPastExpiration(
  grant: ComplimentaryGrantWindow,
  now: Date = new Date()
): boolean {
  if (grant.status !== "ACTIVE" && grant.status !== "INVITED") return false;
 ...
 * 2. ACTIVE Master Admin grant (INVITED never unlocks paid modules)
 * 3. Legacy non-Stripe admin SKU assign (coexistence)
 * 4. Fail closed
 */
export function resolveCommercialEntitlement(input: {
  ...
  if (
    grant &&
    isEntitlementActiveComplimentaryGrant(grant, now) &&
    isProductSku(grant.plan_granted)
  ) {
