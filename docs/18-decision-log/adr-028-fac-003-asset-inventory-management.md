...
9. **Implementation is not authorized** while this ADR is **Proposed** and docs/102 is **Draft**. After Accept + docs/102 Approved + Owner implement authorization, implement only Phase 1. Material scope changes restart Design → Document → Approve. No Production apply, billing, or Stripe changes without Owner authorization.
