/**
 * Landing-only illustrative Mission Control concept shell.
 * Prefer LandingMissionControlScreenshot when real product proof is shown.
 * This remains available as a clearly labeled illustrative layout only.
 */
    ...
    >
      <div className="border-b border-[var(--mpa-color-border-subtle)] bg-[var(--mpa-color-bg-surface)] px-4 py-3 md:px-5">
        <div className="flex flex-wrap items-center justify-between gap-2">
          ...
          <span className="rounded-md border border-[var(--mpa-color-border-default)] bg-[var(--mpa-color-bg-subtle,#F7F8FA)] px-2 py-1 text-xs font-semibold text-[var(--mpa-color-text-secondary)]">
            Illustrative layout
          </span>
        </div>
      ...
      <div className="space-y-3 p-3 md:p-4">
        <div className="rounded-md border border-[var(--mpa-color-border-default)] bg-[var(--mpa-color-bg-surface)] p-3">
          ...
          <ul className="mt-2 grid grid-cols-2 gap-2 sm:grid-cols-4">
            ...
            <li className="col-span-2 rounded-md border border-[var(--mpa-color-brand-primary)]/25 bg-[var(--mpa-color-brand-primary-subtle,#E6F4EF)] px-2.5 py-2 sm:col-span-1">
              <p className="text-xs font-semibold uppercase tracking-wide text-[var(--mpa-color-brand-primary)]">
                Do next
              </p>
              <p className="mt-1 text-sm font-medium leading-snug">Assign open maintenance work</p>
            </li>
            <li className="col-span-2 rounded-md border border-[var(--mpa-color-border-default)] px-2.5 py-2 sm:col-span-1">
              <p className="text-xs font-semibold uppercase tracking-wide text-[var(--mpa-color-text-muted)]">
                Health
              </p>
              <p className="mt-1 text-sm font-medium leading-snug">Needs attention</p>
            </li>
          </ul>
        </div>

        <div className="rounded-md border border-[var(--mpa-color-border-default)] bg-[var(--mpa-color-bg-surface)] p-3">
          <p className="text-sm font-semibold">Attention queue</p>
          <p className="mt-0.5 text-sm text-[var(--mpa-color-text-secondary)]">
            Prioritized operational signals — act from here.
          </p>
