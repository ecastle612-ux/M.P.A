...
export function LandingHeroProductVisual({
  ...
  const panel = (
    <div className="overflow-hidden rounded-tl-xl border border-white/25 bg-[#F4F7F6] shadow-[0_28px_70px_rgba(0,0,0,0.38)]">
      ...
      <div
        className={
          placement === "mobile"
            ...
            : "grid grid-cols-[1.15fr_0.85fr] gap-3 p-3"
        }
      >
  ...
  if (placement === "mobile") {
    return (
      <div className="relative z-10 mx-auto w-full max-w-6xl px-4 pb-10 xl:hidden md:px-6">
        <div className="motion-safe:animate-[mpa-rise_900ms_ease-out]">{panel}</div>
      </div>
    );
  }
  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-x-0 bottom-0 top-8 origin-bottom-right scale-[0.98] motion-safe:animate-[mpa-rise_900ms_ease-out]">
        {panel}
      </div>
    </div>
  );
