...
type E2Report = {
  ...
  assets: Array<{ id: string; name: string; status: string; criticality: string }>;
  systems: Array<{ id: string; name: string; status: string; system_type: string }>;
...
export function E2CertificationPanel() {
  const [organizationId, setOrganizationId] = useState("");
  const [report, setReport] = useState<E2Report | null>(null);
  ...
  async function load() {
    ...
    try {
      const response = await fetch(
        `/api/admin/facility/e2?organizationId=${encodeURIComponent(organizationId.trim())}`
      );
      const payload = await response.json();
      if (!response.ok) {
        throw new Error(payload.error ?? "Failed to load E.2 evidence");
      }
      setReport(payload as E2Report);
    } catch (err) {
      setError(err instanceof Error ? err.message : "Failed to load E.2 evidence");
    } finally {
      setLoading(false);
    }
  }
  ...
  return (
    <section className="max-w-3xl space-y-4 rounded-md border border-[var(--mpa-color-border-default)] bg-white p-4">
      <div>
        <h2 className="text-base font-semibold">FAC-OPS-001 · Phase E.2 certification</h2>
        <p className="mt-1 text-sm text-[var(--mpa-color-text-secondary)]">
          Verify asset creation/lifecycle, site/property linkage, building systems, search, timeline,
          audit, Assistant, and Mission Control signals.
        </p>
      </div>
      <div className="flex flex-wrap items-end gap-2">
        ...
        <Button
          ...
          onClick={() => void load()}
        >
          {loading ? "Loading…" : "Load E.2 evidence"}
        </Button>
      </div>
        ...
        <div className="space-y-3 text-sm">
          <div className="flex flex-wrap items-center gap-2">
            <Badge variant={allPass ? "success" : "danger"}>
              {allPass ? "E.2 evidence Pass" : "E.2 evidence incomplete"}
            </Badge>
            <span className="text-[var(--mpa-color-text-secondary)]">
              {report.assetCount} assets · {report.systemCount} systems · Assistant:{" "}
