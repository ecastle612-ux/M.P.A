...
export function J4CertificationPanel() {
  const [organizationId, setOrganizationId] = useState("");
  const [report, setReport] = useState<J4Report | null>(null);
  ...
  async function load() {
    ...
    try {
      const response = await fetch(
        `/api/admin/launch/j4?organizationId=${encodeURIComponent(organizationId.trim())}`
      );
      const payload = await response.json();
      if (!response.ok) {
        throw new Error(payload.error ?? "Failed to load J4 evidence");
      }
      setReport(payload as J4Report);
    } catch (err) {
      setError(err instanceof Error ? err.message : "Failed to load J4 evidence");
    } finally {
      setLoading(false);
    }
  }
  ...
  const corePass =
    report &&
    report.checks["leaseCreated"] &&
    report.checks["documentGenerated"] &&
    report.checks["leaseSigned"] &&
        ...
        <div className="space-y-3 text-sm">
          <div className="flex flex-wrap items-center gap-2">
            <Badge variant={corePass ? "success" : "danger"}>
              {corePass ? "J4 evidence Pass" : "J4 evidence incomplete"}
            </Badge>
            <Badge variant={report.signWellConfigured ? "success" : "warning"}>
              SignWell {report.signWellConfigured ? "configured" : "not configured"}
            </Badge>
          ...
          {report.signWellNote ? (
            <p className="text-xs text-[var(--mpa-color-text-secondary)]">{report.signWellNote}</p>
          ) : null}
          ...
          <div>
            ...
            <ul className="mt-1 space-y-1 text-[var(--mpa-color-text-secondary)]">
              {report.leases.map((lease) => (
                <li key={lease.id}>
                  {lease.id} · {lease.status} · {lease.signing_channel ?? "no channel"} · SignWell{" "}
                  {lease.signwell_status ?? "n/a"}
                </li>
              ))}
            </ul>
          </div>
