...
export function J7CertificationPanel() {
  const [organizationId, setOrganizationId] = useState("");
  const [report, setReport] = useState<J7Report | null>(null);
  ...
  async function load() {
    ...
    try {
      const response = await fetch(
        `/api/admin/launch/j7?organizationId=${encodeURIComponent(organizationId.trim())}`
      );
      const payload = await response.json();
      if (!response.ok) {
        throw new Error(payload.error ?? "Failed to load J7 evidence");
      }
      setReport(payload as J7Report);
    } catch (err) {
      setError(err instanceof Error ? err.message : "Failed to load J7 evidence");
    } finally {
      setLoading(false);
    }
  }
  ...
  const corePass =
    report &&
    report.checks["dailyOpsReviewed"] &&
    report.checks["timelineEvent"] &&
    report.checks["auditEvent"] &&
    report.checks["nextIsOwnerPortfolio"] &&
    report.assistantRecommendation === "Review your owner's portfolio.";
    ...
    <section className="max-w-3xl space-y-4 rounded-md border border-[var(--mpa-color-border-default)] bg-white p-4">
      <div>
        <h2 className="text-base font-semibold text-[var(--mpa-color-text-primary)]">
          LAUNCH-001 · J7 certification
        </h2>
        <p className="mt-1 text-sm text-[var(--mpa-color-text-secondary)]">
          Verify Mission Control daily briefing, Assistant summary, attention queues, Quick Actions,
          timeline/audit, and next journey → Review your owner&apos;s portfolio.
        </p>
      </div>
      <div className="flex flex-wrap items-end gap-2">
        ...
        <Button type="button" disabled={loading || organizationId.trim().length < 32} onClick={() => void load()}>
          {loading ? "Loading…" : "Load J7 evidence"}
        </Button>
      </div>
        ...
        <div className="space-y-3 text-sm">
          <div className="flex flex-wrap items-center gap-2">
            <Badge variant={corePass ? "success" : "danger"}>
              {corePass ? "J7 evidence Pass" : "J7 evidence incomplete"}
            </Badge>
