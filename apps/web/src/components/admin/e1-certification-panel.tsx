...
type E1Report = {
  ...
  sites: Array<{ id: string; name: string; status: string; timezone: string; created_at: string }>;
  ...
  notifications: Array<{ id: string; notification_key: string; title: string; created_at: string }>;
  checks: Record<string, boolean>;
  assistantRecommendation: string;
};

export function E1CertificationPanel() {
  const [organizationId, setOrganizationId] = useState("");
  const [report, setReport] = useState<E1Report | null>(null);
  ...
  async function load() {
    ...
    try {
      const response = await fetch(
        `/api/admin/facility/e1?organizationId=${encodeURIComponent(organizationId.trim())}`
      );
      const payload = await response.json();
      if (!response.ok) {
        throw new Error(payload.error ?? "Failed to load E.1 evidence");
      }
      setReport(payload as E1Report);
    } catch (err) {
      setError(err instanceof Error ? err.message : "Failed to load E.1 evidence");
    } finally {
      setLoading(false);
    }
  }
  ...
  const allPass =
    ...
    report.activeSiteCount > 0 &&
    report.assistantRecommendation.includes("ready");
    ...
    <section className="max-w-3xl space-y-4 rounded-md border border-[var(--mpa-color-border-default)] bg-white p-4">
      <div>
        <h2 className="text-base font-semibold text-[var(--mpa-color-text-primary)]">
          FAC-OPS-001 · Phase E.1 certification
        </h2>
        <p className="mt-1 text-sm text-[var(--mpa-color-text-secondary)]">
          Verify Facility navigation targets, Mission Control attention, site lifecycle, timeline,
          audit, notifications, search readiness, and Assistant recommendation.
        </p>
      </div>
      <div className="flex flex-wrap items-end gap-2">
        ...
        <Button type="button" disabled={loading || organizationId.trim().length < 32} onClick={() => void load()}>
          {loading ? "Loading…" : "Load E.1 evidence"}
        </Button>
      </div>
        ...
        <div className="space-y-3 text-sm">
          <div className="flex flex-wrap items-center gap-2">
            <Badge variant={allPass ? "success" : "danger"}>
              {allPass ? "E.1 evidence Pass" : "E.1 evidence incomplete"}
            </Badge>
            <span className="text-[var(--mpa-color-text-secondary)]">
              {report.activeSiteCount}/{report.siteCount} active · Assistant:{" "}
          ...
          <div>
            ...
            <ul className="mt-1 space-y-1 text-[var(--mpa-color-text-secondary)]">
              {report.sites.map((site) => (
                <li key={site.id}>
                  {site.name} · {site.status} · {site.timezone} · {site.id}
                </li>
              ))}
            </ul>
          </div>
