...
export function J3CertificationPanel() {
  const [organizationId, setOrganizationId] = useState("");
  const [report, setReport] = useState<J3Report | null>(null);
  ...
  async function load() {
    ...
    try {
      const response = await fetch(
        `/api/admin/launch/j3?organizationId=${encodeURIComponent(organizationId.trim())}`
      );
      const payload = await response.json();
      if (!response.ok) {
        throw new Error(payload.error ?? "Failed to load J3 evidence");
      }
      setReport(payload as J3Report);
    } catch (err) {
      setError(err instanceof Error ? err.message : "Failed to load J3 evidence");
    } finally {
      setLoading(false);
    }
  }
  ...
  const corePass =
    report &&
    report.checks["residentCreated"] &&
    report.checks["propertyAssigned"] &&
    report.checks["unitAssigned"] &&
    report.checks["statusPendingLease"] &&
    report.checks["portalPendingActivation"] &&
    report.checks["timelineEvent"] &&
    report.checks["auditEvent"] &&
    report.checks["residentReady"] &&
    report.assistantRecommendation === "Create your first lease.";
    ...
    <section className="max-w-3xl space-y-4 rounded-md border border-[var(--mpa-color-border-default)] bg-white p-4">
      <div>
        <h2 className="text-base font-semibold text-[var(--mpa-color-text-primary)]">
          LAUNCH-001 · J3 certification
        </h2>
        <p className="mt-1 text-sm text-[var(--mpa-color-text-secondary)]">
          Verify resident creation, property/unit assignment, status, portal Pending Activation,
          timeline, audit, and Mission Control progression to “Create your first lease.”
        </p>
      </div>
      <div className="flex flex-wrap items-end gap-2">
        ...
        <Button type="button" disabled={loading || organizationId.trim().length < 32} onClick={() => void load()}>
          {loading ? "Loading…" : "Load J3 evidence"}
        </Button>
      </div>
        ...
        <div className="space-y-3 text-sm">
          <div className="flex flex-wrap items-center gap-2">
            <Badge variant={corePass ? "success" : "danger"}>
              {corePass ? "J3 evidence Pass" : "J3 evidence incomplete"}
            </Badge>
            <span className="text-[var(--mpa-color-text-secondary)]">
              Assistant: {report.assistantRecommendation}
            </span>
          </div>
          {report.firstResident ? (
            <p className="text-xs text-[var(--mpa-color-text-secondary)]">
              First resident: {report.firstResident.displayName} · {report.firstResident.status} ·
              portal {report.firstResident.portalStatus}
            </p>
          ) : null}
          ...
          <div>
            <h3 className="font-medium">Residents</h3>
            <ul className="mt-1 space-y-1 text-[var(--mpa-color-text-secondary)]">
              {report.residents.map((resident) => (
                <li key={resident.id}>
                  {resident.display_name} · {resident.email} · {resident.status} · portal{" "}
                  {resident.portal_status}
                </li>
              ))}
            </ul>
          </div>
