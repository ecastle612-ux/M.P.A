...
export async function GET(request: Request) {
  try {
    ...
    if (
      ...
      options.status = status as ChargeStatus | "all";
    }
...
export async function POST(request: Request) {
  try {
    ...
    if (!input) return apiError(400, "INVALID_PAYLOAD", "Invalid rent charge payload");

    const charge = await createRentCharge(organizationId, user.id, input, supabase);
    return NextResponse.json({ charge }, { status: 201 });
  } catch (error) {
    const message = error instanceof Error ? error.message : "Rent charge creation failed";
    return apiError(400, "RENT_CHARGE_CREATE_FAILED", message);
  }
}
