...
/** CORE-004 Phase 3 — advance canonical leasing workflow on an applicant carrier. */
export async function POST(
  ...
  try {
    ...
    const canApplicant = evaluatePermission(authorization, "applicant:update");
    const canLease = evaluatePermission(authorization, "lease:update");
    if (!canApplicant && !canLease) {
