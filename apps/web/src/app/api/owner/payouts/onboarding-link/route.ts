...
import { createOwnerOnboardingLink } from "../../../../../lib/owner-payouts/service";
...
export async function POST(request: Request) {
  try {
    ...
    if (!evaluatePermission(authorization, "payout:onboard")) {
    ...
    const returnPath =
      typeof payload["returnPath"] === "string" ? payload["returnPath"] : "/portal/owner/financials";
    const refreshPath =
      typeof payload["refreshPath"] === "string" ? payload["refreshPath"] : returnPath;

    const result = await createOwnerOnboardingLink({
      organizationId,
      ownerUserId: user.id,
