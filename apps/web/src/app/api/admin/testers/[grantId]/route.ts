...
export async function PATCH(request: Request, context: RouteContext) {
  ...
  if (!payload || (payload.action !== "extend" && payload.action !== "resend")) {
    return NextResponse.json({ error: "action must be extend or resend" }, { status: 400 });
  }

  if (payload.action === "resend") {
    const result = await resendComplimentaryInvitation({
