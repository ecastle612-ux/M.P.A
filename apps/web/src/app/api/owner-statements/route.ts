...
import { parseGenerateOwnerStatementInput, type StatementStatus } from "../../../lib/financial/contracts";
import {
...
export async function GET(request: Request) {
  try {
    ...
    if (status === "draft" || status === "generated" || status === "sent" || status === "archived" || status === "all") {
      options.status = status as StatementStatus | "all";
    }
...
export async function POST(request: Request) {
  try {
    ...
    return apiError(400, "OWNER_STATEMENT_CREATE_FAILED", message);
  }
}
