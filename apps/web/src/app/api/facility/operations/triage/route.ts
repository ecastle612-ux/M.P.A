...
import { triageWorkOrder } from "../../../../../lib/maintenance/maintenance-service";
...
export async function POST(request: Request) {
  ...
  const parsed = triageWorkOrderInputSchema.safeParse(payload);
  if (!parsed.success) {
  ...
  try {
    ...
    const workOrder = await triageWorkOrder(
      authz.supabase,
      authz.organizationId,
      authz.user.id,
      parsed.data
    ...
    return NextResponse.json(
      { error: error instanceof Error ? error.message : "Failed to triage facility work order" },
      { status: 400 }
    );
  }
}
