...
describe("SEC-001 SignWell webhook document correlation", () => {
  ...
  function leaseClient(rows: Array<{ id: string; organization_id: string; status: string }>) {
    return {
      from: (table: string) => {
  ...
  it("looks up by signwell_document_id and activates the matching lease", async () => {
    vi.doMock("../../../../../lib/supabase/service-role", () => ({
      createServiceRoleClient: () =>
        leaseClient([{ id: "lease-a", organization_id: "org-a", status: "pending_signature" }])
    }));
  ...
  it("does not activate another lease when metadata lease_id is wrong", async () => {
    ...
    expect(body.reason).toBe("metadata_mismatch");
    expect(activateMock).not.toHaveBeenCalled();
  });
  ...
  it("replays remain idempotent", async () => {
    ...
    vi.doMock("../../../../../lib/supabase/service-role", () => ({
      createServiceRoleClient: () =>
        leaseClient([{ id: "lease-a", organization_id: "org-a", status: "active" }])
    }));
