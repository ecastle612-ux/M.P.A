...
import { PARTNER_PORTAL_ANALYTICS_EVENTS, PARTNER_REQUEST_MAX_BYTES, partnerPropertyUnitHint } from "@mpa/shared";
import { loadPartnerRequestDeps } from "../../../../../../lib/partners/request-deps";
import { publicPartnerUnavailable } from "../../../../../../lib/partners/request-service";
import { resolveLivePropertyPortal } from "../../../../../../lib/partners/property-portal-service";
import { submitPartnerPropertyServiceRequest } from "../../../../../../lib/partners/request-service";
import { consumeRateLimit, requestActorKey } from "../../../../../../lib/security/durable-rate-limit";
import { trackEvent } from "../../../../../../lib/observability/analytics";

export const runtime = "nodejs";

type Params = { params: Promise<{ slug: string; propertySlug: string }> };
...
export async function GET(request: Request, context: Params) {
  const { slug, propertySlug } = await context.params;
  if (
    !(await consumeRateLimit({
      class: "PUBLIC",
      key: `partner-property-portal-get:${slug}:${propertySlug}:${requestActorKey(request)}`
    }))
  ) {
  ...
  try {
    ...
    const live = await resolveLivePropertyPortal(slug, propertySlug, deps);
    if (!live) return unavailable();
    trackEvent({
      eventName: PARTNER_PORTAL_ANALYTICS_EVENTS.portal_viewed,
      properties: { route: `/request/${slug}/${propertySlug}` }
    });
...
export async function POST(request: Request, context: Params) {
  ...
  if (
    !(await consumeRateLimit({
      class: "PUBLIC",
      key: `partner-property-portal-post:${slug}:${propertySlug}:${requestActorKey(request)}`
    }))
  ) {
    return NextResponse.json({ error: "Too many attempts. Try again later." }, { status: 429 });
  }
  ...
  try {
    ...
    const result = await submitPartnerPropertyServiceRequest(slug, propertySlug, payload, deps);
    if (!result.ok) {
      return NextResponse.json({ error: result.error }, { status: result.unavailable ? 404 : 400 });
    }
    if (!result.spam) {
      trackEvent({
